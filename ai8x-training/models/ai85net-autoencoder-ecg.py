...
class CNN_BASE(nn.Module):
    ...
    """
    def __init__(self,
                 num_channels=1,  # pylint: disable=unused-argument
...
class AI85AutoEncoder(CNN_BASE):
    ...
    Input Shape: [BATCH_SZ, FEATURE_LEN, N_AXES] -> [BATCH_SZ, 187, 1] = [N, N_CHANNELS, SIGNAL_LEN]
    """
    ...
    def forward(self, x, return_bottleneck=False):
        ...
        x = self.en_conv1(x)
        # print("output conv1: {}".format(x.shape))
        x = self.en_conv2(x)
        # print("output conv2: {}".format(x.shape))
        x = x.view(x.shape[0], x.shape[1])
        # print("output view: {}".format(x.shape))
        x = self.en_lin1(x)
        # print("output en_lin1: {}".format(x.shape))
        x = self.en_lin2(x)
        # print("output en_lin2: {}".format(x.shape))
...
def ai85autoencoder_ecg(pretrained=False, **kwargs):
    """
